...
(function(){
	var mF={
		defConfig:{//全局默认设置
			...
			index:0,//开始的图片序号
			waiting:20,//Loadin画面最长等待时间
			delay:100,//触发切换模式中切换延迟
			css:true,//用程序定义CSS
			path:'js/',//pattern文件的路径,相对html文件的路径,设置为false或0即取消程序引入
			autoZoom:false//是否让图片自动缩放居中
		},
		pattern:{},
		extend:function(){
	...
	Init={
		...
		initCSS:function(p){
			...
			if(p.css) css.push('.'+p.S+' *{margin:0;padding:0;border:0;list-style:none;}.'+p.S+'{position:relative;width:'+w+'px;height:'+h+'px;overflow:hidden; text-align:left;background:#fff;visibility:visible!important;}.'+p.S+' .loading{position:absolute;z-index:9999;width:100%;height:100%;color:#666;text-align:center;padding-top:'+0.3*h+'px;background:#fff url(http://www.cosmissy.com/myfocus/img/loading.gif) center '+0.4*h+'px no-repeat;}.'+p.S+' .pic{position:relative;width:'+w+'px;height:'+h+'px;overflow:hidden;}.'+p.S+' .txt li,.'+p.S+' .txt li span,.'+p.S+' .txt-bg{width:'+w+'px;height:'+p.txtHeight+'px!important;line-height:'+p.txtHeight+'px!important;overflow:hidden;}.'+p.S+' .txt li p a{display:inline;}');
